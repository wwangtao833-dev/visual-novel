...
当前收录 **40 部代表作品、75 个平台版本**，覆盖 1980—2025 年，含 1982—1985 年早期作品。此项目不是完整历史清单；早期作品缺乏可靠版本信息的地方保持“待核实”。记录以日本发行时间为主；跨地区发行未逐一列出。
...
- [`data/versions.csv`](data/versions.csv)：一行一个版本，含作品字段与核查来源；适合 Excel。
- `index.html`、`app.mjs`、`filter.mjs`、`styles.css`：无需后端和构建工具的静态检索站。
- `scripts/validate_data.py`：校验数据结构和版本年份。已提交的 JSON/CSV 为公开数据源。
...
3. 同步维护 CSV；运行 `python3 scripts/validate_data.py`，再检查页面筛选是否显示新版本。
